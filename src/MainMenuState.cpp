...
pions de sa couleur à la fin de la partie.");
  ...
  mGameDescs.push_back("Dans Puissance 4, vous devez faire une ligne de 4 pions de même couleur. Dans cette variante, chaque ligne vous rapporte 1 point et disparait ensuite.");
  ...
  mGameDescs.push_back("Dans cette variante de 5 or more, vous ne déplacez pas les pions. Vous changez seulement leur couleur !!");
  mGameDescs.push_back("Dans Morpion, il faut aligner 3 pions de la même couleur");  
  ...
  mGameDescs.push_back("Dans Chomp, vous mangez des morceaux de choclat : tous ceux en haut et à droite de la case choisie. Celui qui mange le dernier bout en bas à gauche a perdu.");  
...
void MainMenuState::handle(const char& c){
  ...
  if(c == 'p' || c == MARK){
    ...
    case 3:
      Game::getInstance()->getHandler().change(new FiveOrMore::Config());
      break;

    case 4:
      Game::getInstance()->getHandler().change(new FiveOrMore_var::Config());
      break;
      
    case 5:
      Game::getInstance()->getHandler().change(new TicTacToe(Color::RED, Color::BLUE) );
      break;
...
void MainMenuState::render(){
  ...
  Console::getInstance()->draw(4, 8, "5 or more - variante à changements");
